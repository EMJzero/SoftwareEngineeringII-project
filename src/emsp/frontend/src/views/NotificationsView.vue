<template>
  <div class="relative w-full items-center justify-center">
    <div class="text-center justify-items-center pt-2">
      <p class="text-white font-semibold text-4xl pt-5 pb-8">Your Notifications</p>
      <div class="text-white font-semibold text-xl space-x-16 pt-10 pb-4">
        <button class="rounded-lg bg-blue-600 py-3 px-16 font-normal text-white hover:bg-blue-700" @click="notification_controller.refreshNotifications()">Refresh</button>
        <button class="rounded-lg bg-red-600 py-3 px-16 font-normal text-white hover:bg-red-700" @click="notification_controller.clearNotifications()">Clear</button>
      </div>
      <p v-if="isLoading" class="text-grey font-semibold text-2xl space-x-16 pt-20">Loading Notifications...</p>
      <ul v-if="!isLoading && notifications && notifications.length > 0" class="list-none pl-4 text-stone-400 text-lg" style="max-width: 500px; margin-left: auto; margin-right: auto">
